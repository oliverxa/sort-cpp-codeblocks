...
void TestSORT(string seqName, bool display)
{
    ...
    // read image file
	string imgPath = "/home/hkuit155/Desktop/Datasets/2DMOT2015/train/" + seqName + "/img1/";
	...
	string detFileName = "/home/hkuit155/Desktop/Datasets/2DMOT2015/train/" + seqName + "/det/det.txt";
	...
	char ch; //using for store block
	float tpx, tpy, tpw, tph; //setting the bbox

	while ( getline(detectionFile, detLine) )
	{
		TrackingBox tb;

        // using istringstream to input the file and out
		ss.str(detLine);
	...
	int maxFrame = 0;
	for (auto tb : detData) // find max frame number    for (auto name : range)
	{
	...
	// re range the frame from 1 to maxFrame
